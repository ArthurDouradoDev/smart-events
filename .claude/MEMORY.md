...
- **Status:** `pytest -m "not vpn"` = 120 passed, 10 skipped; `node --check` OK nos JS. Pendente: validar AO VIVO na VPN (`HttpCollector.collect_alarms` real devolve só os tipos filtrados — confirmar que o `roarand` do módulo monitoring serve ao `fmwebsite`) e smoke visual do painel.
- **Fora do MVP (não implementado):** correlação alarme→site do evento, alertas derivados de alarmes Critical com silenciamento, perfis de filtro.
