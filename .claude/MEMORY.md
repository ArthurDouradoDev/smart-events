...
### [Sessão 2026-06-30] Coletor de alarmes COM filtro por tipo (`imaster_alarms.py`)
- **Motivação:** `alarm.py` baixa TODOS os alarmes ativos; faltava coletar só os tipos escolhidos (ex.: VSWR + Cell Unavailable), como a tela "Current Alarms" faz com filtro. Implementação completa do plano em `implementacao-filtro-alarmes.md`.
- **Mecanismo (3 comandos no mesmo endpoint `POST /rest/fmwebsite/v1/commands`):** o iManager NÃO filtra ao listar — filtra ao **criar uma visão** no servidor (`cmd 1102`, devolve um `modelID` novo) e depois só pagina essa visão (`cmd 1103`). `cmd 2909` (tradução de nomes de grupo) é opcional e foi ignorado, pois o `alarmName` já vem no `1103`.
- **Filtro = pares `{alarmId, alarmGroupId}`, não só `alarmId`:** o mesmo alarme existe cadastrado em várias famílias de equipamento (uma por `alarmGroupId`), e até com `alarmId` diferente por grupo.
- **Catálogo rico (decisão-chave, supera o plano):** o operador exporta do próprio sistema o CSV `Alarm Group ID, Alarm Group Name, Alarm ID, Alarm Name, Alarm Severity` (BOM, ~7059 linhas, 2308 nomes distintos, **23 grupos**). Salvo com nome estável em `alarms/catalogo-alarmes.csv` (re-exportar e sobrescrever quando a base mudar). Com ele montamos os **pares reais e exatos** por nome — **abandonado o produto cartesiano "por chute" com 10 grupos** que o plano sugeria (era incompleto: faltavam BSC6910UMTS, MICRO BTS3900, os Pools, ECNS…). Ex.: VSWR → 11 pares (HAR só tinha 10), Cell Unavailable → 5. `lista-alarmes.csv` (antigo, só nome→id) ficou superado mas não foi removido.
- **`imaster_alarms.py`** (standalone, espelha config/sessão/tokens de `alarm.py`), blocos: `load_catalog` (DictReader → `{nome: [(alarmId, alarmGroupId), …]}`), `resolve_pairs` (nomes → pares dedup, valida inexistente → `ValueError`), `build_condition(pairs)`, `create_model` (cmd 1102, `modelID:null` + `_extract_model_id` robusto p/ vários caminhos na resposta), `fetch_page`/`collect_all` (cmd 1103 paginado, `autoRefresh:false` + dedup por `csn`), `flatten`, `pull_alarms` (→ DataFrame), `harvest_groups` (coleta sem filtro p/ descobrir grupos reais). CLI: `--alarms`, `--list`, `--harvest-groups`; padrão = VSWR + Cell Unavailable.
- **`alarm.py` inalterado** (coletor sem filtro). `imaster_alarms.py` segue standalone com IP/tokens fixos colados do DevTools (sem integração com o app, mesma decisão do operador para `alarm.py`).
- **Sessão expirada:** `_check_session_alive` (chamado ANTES de `raise_for_status`) cobre HTTP 401/403 e o redirect SSO (`unisso`/`login.action`, que volta 200 com HTML longo) → `RuntimeError` com instrução de atualizar os tokens.
- **`bspSessionId` pode ir VAZIO (`""`):** descoberto ao vivo — o servidor só o ecoa no sufixo `@` do `modelID`, e o modelID é criado e consumido na mesma execução. Por isso o cURL do DevTools (cmd 8100) NÃO traz bspSessionId e não precisa: bastam `bspsession` (cookie) + `roarand` (header). `BSP_SESSION_ID = ""` no topo do arquivo.
- **Caminho do `modelID` confirmado ao vivo:** resposta do 1102 = `{"parameters":{"jobId":...,"modelID":"000.…@","bspSessionId":""},"status":1}`. `_extract_model_id` lê `parameters.modelID` ✓.
- **VALIDADO AO VIVO (VPN, 30/06):** com tokens frescos, pipeline 1102→1103 ponta a ponta OK. Filtro padrão (VSWR+Cell Unavailable, 16 pares) → 1103 devolveu SÓ esses 2 alarmId (zero contaminação). Coleta completa via `python imaster_alarms.py` = **697 alarmes** (Cell Unavailable 371 + RF Unit VSWR 326; Major 608/Minor 89, batendo com o `levelResults`), 0 `csn` duplicado, CSV+XLSX exportados com 19 colunas. **Tokens são voláteis** — re-colar do DevTools quando expirar (401). Próximo passo opcional: `--harvest-groups` p/ cruzar com os 23 grupos do catálogo.
...
### [Sessão 2026-07-01] Integração de alarmes (coleta filtrada por tipo dentro do app)
- **Motivação:** trazer a coleta de alarmes filtrados (antes standalone em `imaster_alarms.py`, validada AO VIVO na VPN em 30/06) para dentro do app, seguindo `Collector → SQLite → Api → bridge → Frontend` e o `plano-integracao-alarmes.md`.
- **Fluxo (portado do standalone, sem import cruzado):** cmd 1102 cria a "visão" filtrada (`modelID`) a partir dos pares `{alarmId, alarmGroupId}`; cmd 1103 pagina (`autoRefresh=False`, foto estável); dedup por `csn`. Endpoint `/rest/fmwebsite/v1/commands` no **mesmo host** do OSS/PM → reusa a sessão `monitoring` do `session.json` (bspsession+roarand). **Nenhuma mudança em `session_renew.py`.**
- **Decisões:** (D1) armazenar no banco **do evento** (`smart_events_<id>.db`), tabela `alarms` com `csn` PK (dedup natural); alarmes são da rede toda, não recortados pelo polígono. (D2) loop agendado `INTERVAL_ALARMS_SECONDS=180` + `collect_alarms_now()` (refresh/mudança de filtro). (D3) filtro por evento em `oss.alarm_filter` (nomes), default = VSWR + Cell Unavailable (`core.collector._DEFAULT_ALARM_NAMES`); sobrevive ao `_sanitize_event` por estar dentro de `oss`. (D4) painel próprio = **drawer** `#alarms-drawer` (padrão dos drawers de alertas/logs), multi-select com busca + contagens por severidade + tabela; não fundido com "Alertas ativos".
- **Catálogo:** `alarms/catalogo-alarmes.csv` (2308 nomes → pares), recurso empacotado read-only (frozen → `sys._MEIPASS`; dev → repo), cacheado 1× via `_load_alarm_catalog()`; adicionado aos `datas` do `main.spec`. Re-exportar do sistema quando a base mudar.
- **Arquivos:** `core/collector.py` (constantes/helpers de alarme + `HttpCollector.collect_alarms` + stubs Mock/Csv/Null), `core/database.py` (tabela `alarms`, `insert_alarms_batch`, `get_alarms`, DELETE no `clear_event_history`), `core/scheduler.py` (thread/loop/status/`collect_alarms_now`), `core/models.py` (`OssConfig.alarm_filter`), `sample_event.json`, `api/api.py` (`get_alarms`/`get_alarm_catalog`/`get_alarm_filter`/`set_alarm_filter`/`refresh_alarms` + `alarms` no `get_collection_status`), `frontend/{index.html,css/main.css,js/{alarms(novo),app,bridge,state}.js}`, `main.spec`, `tests/test_alarms.py` (novo) + teste VPN em `tests/test_http_vpn.py`.
- **Fuso dos alarmes (corrigido):** `arriveUtc`/`occurUtc` vêm como `"YYYY-MM-DD HH:MM:SS"` **no fuso do cliente** (cookies `timemode=client`/`timezone=America/Sao_Paulo`), apesar do nome "Utc" — comprovado nos HAR (`alarms/10.220.50.9.new.har`, capturado ~15:42 local com alarmes chegando a cada segundos). Guardar cru fazia o front (que adiciona `Z` e trata como UTC) exibir "3h atrás". `_flatten_alarms` agora converte local→UTC reusando `_parse_trace_timestamp` (`oss.timezone_offset_min`, default −180); banco guarda UTC real.
- **Correlação alarme→site (implementada — nosso lado):** `Api.get_alarms` marca cada alarme com `in_event`/`serving_site`/`serving_site_name` casando o `source` (meName) com os sites do evento (igualdade/prefixo/substring de id/nome, via `_resolve_site_for_source`), em query time (como os VIPs) — não recorta a coleta (continua da rede toda). Painel: toggle "Somente sites do evento" (default LIGADO); badge do header conta críticos **no evento**. Ressalva: depende do meName casar com o id/nome do site (eventos reais usam `SR-XXXX` → casa).
- **Status:** `pytest -m "not vpn"` = 123 passed, 10 skipped; `node --check` OK nos JS. Pendente: validar AO VIVO na VPN (`collect_alarms` real devolve só os tipos filtrados + fuso correto — confirmar que o `roarand` do módulo monitoring serve ao `fmwebsite`) e smoke visual do painel.
- **Fora do MVP (não implementado):** destaque no mapa de sites com alarme, alertas derivados de alarmes Critical com silenciamento, perfis de filtro.
